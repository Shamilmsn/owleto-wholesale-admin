...
{{--    </div>--}}
    ...
    {!! Form::label('package_delivery_times', trans('lang.package_delivery_times'), ['class' => 'col-4 control-label']) !!}
    <div class="col-8">
        @if(isset($order->packageOrders[0]))
            @foreach($order->packageOrders[0]->package->package_delivery_times as $packageDeliveryTime)
                {{ $loop->first ? '' : ', ' }}{{ $packageDeliveryTime->name  }}
            @endforeach
...
<div class="form-group row col-md-4 col-sm-12">
    ...
    <div class="col-8">
        @if(isset($order->packageOrders[0]))
            ...
            <p>{!! $order->packageOrders[0]->package->market->name !!}</p>
        @endif
    </div>
    ...
    <div class="col-8">
        @if(isset($order->packageOrders[0]))
            <p>{!! $order->packageOrders[0]->package->market->address !!}</p>
        @endif
    </div>
    ...
    <div class="col-8">
        @if(isset($order->packageOrders[0]))
            ...
            <p>{!! $order->packageOrders[0]->package->market->phone !!}</p>
        @endif
    </div>

    {!! Form::label('package_days', trans('lang.package_days'), ['class' => 'col-4 control-label']) !!}
    <div class="col-8">
        @if(isset($order->packageOrders[0]))
            @foreach($order->packageOrders[0]->package->package_days as $packageDay)
                {{ $loop->first ? '' : ', ' }}{{ $packageDay->name  }}
            @endforeach
