...
class DriversPayoutDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('paid_date', function ($drivers_payout) {
                return getDateColumn($drivers_payout, 'paid_date');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            [
                'data' => 'amount',
                'title' => trans('lang.drivers_payout_amount')."<span>" . ' '.   setting('default_currency') . "</span>",
     ...
     *
     * @param \App\Models\DriversPayout $model
    ...
    public function query(DriversPayout $model)
    {
        ...
        if(auth()->user()->hasRole('admin')){
            return $model->newQuery()
                ->with("user")
                ->join('users', 'users.id', '=', 'drivers_payouts.user_id')
//                ->where('users.city_id', $user->city_id)
                ->select('drivers_payouts.*');

        }elseif(auth()->user()->hasRole('driver')){

            return $model->newQuery()
                ->with("user")
                ->where('drivers_payouts.user_id',auth()->id())
                ->select('drivers_payouts.*');
        }
    }
