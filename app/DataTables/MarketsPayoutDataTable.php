...
class MarketsPayoutDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->editColumn('paid_date', function ($markets_payout) {
                return getDateColumn($markets_payout, 'paid_date');
            })
    ...
    protected function getColumns()
    {
        $columns = [
            [
                'data' => 'market.name',
                'title' => trans('lang.markets_payout_market_name'),
            ...
            [
                'data' => 'amount',
                'title' => trans('lang.markets_payout_amount')."<span>" . ' '.   setting('default_currency') . "</span>",
     ...
     *
     * @param \App\Models\MarketsPayout $model
    ...
    public function query(MarketsPayout $model)
    {
        ...
        if(auth()->user()->hasRole('admin')){
            return $model->newQuery()
                ->with("market")
                ->join('markets', 'markets.id', '=', 'markets_payouts.market_id')
                ->where('markets.city_id', $user->city_id)
                ->select('markets_payouts.*');
        }
