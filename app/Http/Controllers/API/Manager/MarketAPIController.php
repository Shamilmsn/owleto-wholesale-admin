...
class MarketAPIController extends Controller
{
    ...
    {
        $this->marketRepository->skipCache();
...
//            $this->marketRepository->pushCriteria(new ActiveCriteria());
    ...
    public function openOrCloseMarket(Request $request)
    {
        $userMarketIds = Market::whereHas('users', function ($query){
            $query->where('user_id', auth()->id());
        })->pluck('id');

        $collection = collect($userMarketIds);

        $exist = $collection->contains($request->market_id);
        ...
        $market->closed = $isClosed;
        $market->save();

        return $this->sendResponse($market, 'Market Updated');
    }
}
