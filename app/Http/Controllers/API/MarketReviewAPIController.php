...
//        $otherInput = $request->except("user_id","market_id");
        try {
            $existingReview = $this->marketReviewRepository->where('market_id',$request->market_id)->where('user_id',$request->user_id)->first();
                if($existingReview) {
                    $marketReview = $this->marketReviewRepository->update($input,$existingReview->id);
                }else{
